...
if(!['810497712381493271'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) 
return message.channel.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`Hey Sen ! ${message.author} bu komutu kullanmak için yetkin bulunmamakta.`)
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
  
const tag = '✭'   
const erkekrol = message.guild.roles.cache.find(r => r.id === '810497712285024266') 
const erkekrol2 = message.guild.roles.cache.find(r => r.id === '810497712268509263')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '810497712268509261')
const genelchat = message.guild.channels.cache.find(c => c.id === '810497714638028825')
const savelog = message.guild.channels.cache.find(c => c.id === '810561563182104586')
...
genelchat.send(`${member} Aramıza katıldı, hoş geldin umarım keyifli vakit geçirirsin. Sunucumuzun \`${tag}\` tagını alarak ailemizin parçası olabilirsin.`)
